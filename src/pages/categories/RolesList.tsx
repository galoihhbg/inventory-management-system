...
export default function RolesList() {
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      notification.error({ message: t('roles.deleteFailed'), description: err?.response?.data?.message || err.message });
    }
  };
  ...
  const columns = [
    ...
    { title: t('common.actions'), key: 'actions', render: (_: any, record: any) => (
      <Space>
        ...
        <Popconfirm title={t('roles.confirmDelete')} onConfirm={() => handleDelete(record.id)}>
