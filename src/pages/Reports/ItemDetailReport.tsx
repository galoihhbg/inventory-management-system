...
import { Table, DatePicker, Button, Space, Card, Select } from 'antd';
import { ReloadOutlined, DownloadOutlined } from '@ant-design/icons';
import { useFilteredList } from '../../api/hooks';
import { ReportFilter, ItemDetailReportItem, Warehouse, Item } from '../../types';
...
export default function ItemDetailReport() {
  ...
  const { 
    ...
    pagination,
    refetch 
  } = useFilteredList<ItemDetailReportItem, ReportFilter>({
    endpoint: '/reports/item-detail',
  ...
  return (
    <div>
      <Card 
        title={t('reports.itemDetailReport')}
      ...
      <Table 
        rowKey={(record, index) => `${record.documentNumber}-${record.itemCode}-${index}`}
        loading={isLoading} 
        dataSource={data} 
        columns={columns} 
        pagination={false}
